...
public class Library
{
    ...
    /**
     * holdBook places a hold on book with given call number for the given member
     * @param callNumber String call number of book to hold
     * @param member Member member who wishes to hold the book.
     * @return boolean
     */
    public boolean holdBook( String callNumber, Member member ) 
	throws Exception {
	
	String sqlQuery = "select count(*) as counted from hold where callnumber='" + callNumber + "' and ssn='" + member.getSSN() + "'" ;
	...
	// if alreadyHeld is true, then don't place a hold.  Place a hold otherwise.
	if (alreadyHeld) {
	    return true;
	} else {
	    sqlQuery = "INSERT INTO Hold VALUES (" + member.getSSN() + ", '" + callNumber + "', CURRENT_TIMESTAMP)";
     ...
     * @param bookId int id of book to check in
     * @return boolean
     */
	...
	// Build the sql query.
	String sqlQuery =  "UPDATE Book SET borrowerssn = null, librarianssn = null, duedate = null WHERE BookID = " + bookId;
	// Run the update.
	if (myConnection.runUpdate( sqlQuery ) > 0)
	    return true;
	else
    ...
    /**
     * approve a check out for book with given book id for a member with given member ssn
     * @param bookId int id of book to check out
     * @param memberSSN int ssn of member who wants to check book out
     * @return boolean
     */
    private boolean approveCheckout( int bookId, int memberSSN ) 
	throws Exception {
	...
	if ( checkedOut ) {
	    ...
	    int availableCopyCount = LibraryBookTitle.getAvailableTitleCopyCount( callNumber );
    ...
    /**
     * checkMemberHold checks in the member with ssn memberSSN has a hold on book with id bookId
     * @param bookId int id of book
     * @param memberSSN int ssn of member
     * @return boolean
     */
    private boolean checkMemberHold( int bookId, int memberSSN )
	throws Exception {
    ...
    /**
     * checkOutBook checks out the book with bookId for the member with ssn memberSSN by the Librarian librarian
     * @param bookId int id of book
     * @param memberSSN int ssn of member
     * @param librarian Librarian librarian who is checking this book out
     * @return boolean
     */
	...
	if ( approveCheckout( bookId, memberSSN ) ) {
	    ...
	    java.util.Date d = new java.util.Date(System.currentTimeMillis() + MILLISECONDS_IN_THREE_WEEKS);
		...
		sqlQuery = new String( "UPDATE Book SET borrowerssn = " + memberSSN +
		...
		sqlQuery[0] ="DELETE FROM hold WHERE ssn=" + memberSSN + " and callnumber='" + callNumber + "'";
		sqlQuery[1] =  "UPDATE Book SET borrowerssn = " + memberSSN +
		    ", librarianssn = " + librarian.getSSN() +
		    ", duedate = '" + dd + "' " +
		    "WHERE bookid = " + bookId;
		return myConnection.runChainedUpdate( sqlQuery, Connection.TRANSACTION_SERIALIZABLE );
    ...
    /**
     * releaseHold releases a hold on booktitle with call number callNumber for the specified member.
     * @param callNumber String call number of hold to release
     * @param member Member member who has the hold
     * @return boolean
     */
    ...
    /**
     * validateLibrarian returns a librarian object for the librarian with the given ssn and password
     * @param ssn String ssn of librarian
     * @param passWord String password of librarian
     * @return Librarian
     */
     ...
     * @param ssn String ssn of member
     * @param passWord String password of member
     * @return Member
     */
    public Member validateMember( int ssn, String passWord ) 
	throws Exception{
     ...
     * @return BookTitleSet
     */
    public BookTitleSet getBookTitles() 
	throws Exception{
	...
	ResultSet r = myConnection.runQuery( sqlQuery );
	while( r.next() ) {
	    currentBooks.addBookTitle( LibraryBookTitle.getBookTitle( r.getString( "callnumber" ) ) );
	}
	return currentBooks;
    }
}   
